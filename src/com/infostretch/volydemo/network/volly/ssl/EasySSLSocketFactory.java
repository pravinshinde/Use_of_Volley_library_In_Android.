package com.infostretch.volydemo.network.volly.ssl;
...
public class EasySSLSocketFactory implements LayeredSocketFactory {
	...
	public static class MyTrustManager implements X509TrustManager {
		...
		public MyTrustManager(KeyStore localKeyStore)
			...
			instream = DemoApp.getDailyUseAppContext().getResources()
					.openRawResource(R.raw.teststore);
		...
		public X509Certificate[] getAcceptedIssuers() {
			if (null == acceptedIssuers) {
				X509Certificate[] acceptedIssuersDefault = defaultTrustManager
						.getAcceptedIssuers();
				X509Certificate[] acceptedIssuersLocal = localTrustManager
						.getAcceptedIssuers();
				acceptedIssuers = new X509Certificate[acceptedIssuersDefault.length
						+ acceptedIssuersLocal.length];

				System.arraycopy(acceptedIssuersDefault, 0, acceptedIssuers, 0,
						acceptedIssuersDefault.length);
				System.arraycopy(acceptedIssuersLocal, 0, acceptedIssuers,
						acceptedIssuersDefault.length,
						acceptedIssuersLocal.length);
			}
	...
	private static SSLContext createIgnoreSSLContext() throws IOException {
		try {
			SSLContext context = SSLContext.getInstance("TLS");
			context.init(null,
					new TrustManager[] { new IgnoreCertTrustManager() }, null);
			return context;
		} catch (Exception e) {
			e.printStackTrace();
			throw new IOException(e.getMessage());
